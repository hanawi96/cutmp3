...
const PitchControl = ({
  ...
  const getPitchBgColor = useCallback((pitch) => {
    ...
    if (pitch === undefined || pitch === null || isNaN(pitch)) {
      console.warn(
        "[PitchControl] getPitchBgColor received invalid value:",
        pitch
      );
  ...
  if (disabled) {
    return (
      <div
        className={`opacity-50 cursor-not-allowed ${classes.container || ""}`}
      >
        <button
          type="button"
          disabled
          className={`flex items-center space-x-2 px-3 py-2 bg-gray-100 text-gray-400 rounded-lg border border-gray-200 ${
            classes.button || ""
          }`}
        >
    ...
    <div className={`relative ${classes.container || ""}`}>
      ...
      >
        <div className="flex items-center space-x-2">
          ...
          >
            <Music
              className={`w-4 h-4 transition-colors duration-200 ${getPitchColor(
                tempPitch
              )}`}
            />
          </div>
          <div className="text-left">
            <div
              className={`text-sm font-medium ${getPitchColor(tempPitch)} ${
                classes.text || ""
              }`}
            >
              {formatPitch(tempPitch)}
            </div>
            <div className="text-xs text-gray-500">semitones</div>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[320px] p-4">
          ...
          <div className="text-center mb-4">
            <div
              className={`text-2xl font-bold transition-colors duration-200 ${getPitchColor(
                tempPitch
              )}`}
            >
              {formatPitch(tempPitch)}
