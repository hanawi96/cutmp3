...
const resetToRegionStart = (source = "unknown") => {
  ...
  const calculateVolumeForProfile = (relPos, profile) => {
    ...
    // If no fade is enabled, use the specified profile
    if (!fadeEnabledRef.current) {
      switch (profile) {
        ...
        case "custom": {
          if (relPos <= 0.5) {
            const t = relPos * 2;
            return intendedVolume * (currentCustomVolume.start + (currentCustomVolume.middle - currentCustomVolume.start) * t);
          } else {
            const t = (relPos - 0.5) * 2;
            return intendedVolume * (currentCustomVolume.middle + (currentCustomVolume.end - currentCustomVolume.middle) * t);
          }
        }
        default:
          return intendedVolume;
    ...
    // Handle fade effects when enabled
    ...
    let fadeVolume = intendedVolume;
    
    if (fadeInRef.current && posInRegion < fadeDuration) {
      fadeVolume *= (posInRegion / fadeDuration);
    }
    
    if (fadeOutRef.current && timeToEnd < fadeDuration) {
      fadeVolume *= (timeToEnd / fadeDuration);
    }
